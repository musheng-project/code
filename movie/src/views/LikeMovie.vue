<template>
  <div class="like-movie">
    <div>
      <van-nav-bar title="我的收藏" left-text="返回" left-arrow @click-left="back" fixed/>
    </div>
    <div>
        ...
        <div class="movie-detail-box">
          <div class="movie-detail-t">
            ...
            <div class="movie-detail-msg fl">
              <div class="clearfix">
                <span class="fl m-title one-text">{{item.titleCn}}</span>
                <span class="fr collection" @click.stop="deleteLikeMovie(item.id)">
                  <van-icon name="delete" />
                </span>
              </div>
              <div class="m-type one-text">{{item.typeInfo}}</div>
              <div class="clearfix m-info">
                <div class="fl">{{item.release.date}}</div>
              <div class="fl">{{item.release.location}}</div>
              <div class="fl">{{item.runTime}}</div>
              </div>
              <div class="star-box clearfix">
                ...
                <!-- 评分 -->
                <div class="fl star-score">{{item.rating}}</div>
              </div>
            </div>
          </div>
        </div>
...
.like-movie {
  ...
  .star-score {
    font-size: 16px;
    font-weight: bold;
    background-image: linear-gradient(45deg, #ea1e63, #ea1e63);
